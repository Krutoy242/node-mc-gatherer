...
function getBucketFluid(
  ...
  if (stack.name.startsWith('minecraft:')) {
    if (stack.name.startsWith('minecraft:water_bucket:0'))
      return 'water'
    if (stack.name.startsWith('minecraft:lava_bucket:0'))
      return 'lava'
    if (stack.name.startsWith('minecraft:milk:0'))
      return 'milk'
    if (stack.name.startsWith('minecraft:milk_bucket:0'))
      return 'milk'
  }
...
function bucketToFluid(
  ...
  ingr.stacks.forEach((stack) => {
    if (stackBucketToFluid(stack, getFullID))
      ingr.amount = 1000
  })
}
  ...
  cat.recipes.forEach((rec) => {
    rec.input.items.forEach((slot) => {
      ...
      slot.stacks.some((stack) => {
        if (stack.type !== 'item' && stack.type !== 'oredict')
          return false
      ...
      if (slot.stacks.length > 20) {
        ...
        for (const [id_meta, g] of Object.entries(_.groupBy(splitted, s => s.id_meta))) {
          if (g.length < 20 || g.filter(i => i.nbtHash).length < 19)
            continue
...
adapters.set(/tinkersjei__tool_stats/, (cat) => {
  ...
  cat.recipes
    ...
    .forEach((rec) => {
      ...
      if (!input)
        throw new Error('Cannot find input for Tinker\'s Tool')
...
adapters.set(/thermalexpansion__insolator$/, (cat) => {
  ...
  cat.recipes.forEach((rec: JEIECustomRecipe) => {
    ...
    if (!rec.input.items.some(s => s.stacks.some(i => i.name === 'thermalfoundation:fertilizer:0')))
      return
...
adapters.set(/forestry__fabricator/, (cat) => {
  cat.recipes.forEach((rec: JEIECustomRecipe) => {
    ...
    if (slot)
      slot.amount = rec.output.items[0].amount / 33 // TODO max durability
...
adapters.set(/THAUMCRAFT_ASPECT_FROM_ITEMSTACK/, (cat) => {
  ...
  cat.recipes.forEach((rec) => {
    rec.input.items.forEach((slot) => {
      ...
      if (inputBlacklist.some(id => itemId.startsWith(id)))
        return
...
adapters.set(/^requious__.*/, (cat, tools) => {
  cat.recipes.forEach((rec: JEIECustomRecipe) => {
    ...
    if (!rec.input.items.length)
      rec.input.items = [getSlot('placeholder:ticks', 2000)]
  })
})
...
function getSpecie(item: JEIEItem, tools: Tools) {
  ...
  if (!specieGenes)
    throw new Error('Cant parse bee genes')
  return specieGenes
}
